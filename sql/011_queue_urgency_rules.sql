-- 011_queue_urgency_rules.sql
-- Define/refine default urgency levels for active_conversation_queue.
--
-- Key refinement:
-- - stale inbound messages that still need reply remain urgent;
-- - 24h follow-ups after our latest outbound become high, not automatically urgent;
-- - follow_up_due_24h is classified before generic needs_reply.
...
COMMENT ON VIEW public.active_conversation_queue IS
  'Active furniture conversation queue for lead/workflow triage with documented default urgency levels. Excludes terminal stages, includes scheduled threads, preview text only, and distinguishes urgent stale inbound work from high-priority outbound follow-up due work.';

COMMIT;

-- Suggested manual readback after apply:
-- SELECT conversation_thread_id, platform, stage, needs_reply, queue_reason, queue_urgency, latest_message_direction, latest_body_preview
